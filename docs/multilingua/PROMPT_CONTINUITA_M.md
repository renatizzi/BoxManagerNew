...
Poi: Sync Gradle se lo chiede; variante **famigliaDebug** (non `play`); **Run** sul telefono. Topbar attesa: **1.3-famigliaB5.21**.

**Ritest B5.21:** Impostazioni → Disco di rete (messaggio «già installata» subito se CIFS c’è; solo Apri l’app / Annulla). Backup → file `*.zip`. Ricevi tabelle → dialogo causa se blocco. Guida: [GUIDA_DISCO_RETE.md](../famiglia/GUIDA_DISCO_RETE.md).
