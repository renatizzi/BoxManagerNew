...
| C3 | A2 Cestino | **IN TEST** | play **1.3.5** — soft-delete, Utility, premium, R6/R8 |
...
2. Renato: `git checkout main` + `git pull origin main` + **Run `playDebug`** → topbar = versione detta dall’agente (ora **`v. 1.3.5`**, senza famiglia). **Mai** altri branch per il test telefono.
...
4. **C3 / A2 Cestino** — **IN TEST** play **1.3.5** (merge main 16/09).
