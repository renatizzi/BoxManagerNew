...
| M4 | Alzare `versionCode` (> ultimo Play) e `versionName` (es. **1.3**) | C3 — topbar ufficiale **1.3**, non etichetta sviluppo `1.3-famigliaB5.x` |
| M4b | Identità «family» | Solo tecnico (flavor/branch); **una** BoxManager su `main`. Non censurare guida/messaggi con «famiglia/familiari» |
