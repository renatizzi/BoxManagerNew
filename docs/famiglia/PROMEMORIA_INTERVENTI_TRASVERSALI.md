...
|----|------|-------------|-------|
| **B-ROTATE-FORM-DRAFT** | 10/09/2026 | In inserimento dati (qualsiasi elemento) la stringa digitata **sparisce** se, prima della conferma, si ruota lo schermo. | **IN CORSO C1** (SI 14/09 apri) — `configChanges` su Main/BoxDetail/Categorie/Luoghi così i dialog non si distruggono in rotazione. Eccezione già ok: username Impostazioni |
| **B-AUTO-FILES-LIST** | 14/09/2026 | Regressione vs versioni precedenti: i **file automatici** non devono comparire insieme a quelli effettivi nella **lista proposta dall’app**; devono restare visibili solo con **Sfoglia cartella**. Vale per Backup/Ripristino (es. `PRE_RESTORE_…` / auto) e, se applicabile, altri elenchi file. | **Da verificare/fix** — SI nota Renato 14/09. In codice: `RestoreActivity.visibleBackupFiles` già filtra `PRE_RESTORE_` se `showSafetyCopies=false`. Controllare su device se la regressione è tornata e su quali schermate |
...
| **B-PRE-PLAY-PHONE-TEST** | 13/09/2026 | Processo ovvio: agente modifica → Renato pull + **Run `playDebug`** (senza firma) → OK test → solo allora AAB **`playRelease`** firmato su Play. Mai Run su `playRelease` per le prove. | **SI Renato 13/09** (ribadito) — SOLO_TU / checklist / runbook / play README |
